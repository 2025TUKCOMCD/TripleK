...
struct_message receivedData; // 수신한 Wi-Fi 데이터 저장
char action[32];

bool wifiCredentialsReceived = false; // Wi-Fi 정보 수신 상태
bool wifiStatus = false; // WiFi 현재 상태 
...
void onDataReceive(const uint8_t *mac, const uint8_t *data, int len) {
  ...
  Serial.print("SSID: ");
  Serial.println(receivedData.ssid);
  Serial.print("Password: ");
  Serial.println(receivedData.password);

  wifiCredentialsReceived = true; // Wi-Fi 정보 수신 완료
}
...
void loop() {
  ...
  if (wifiCredentialsReceived) {
    wifiCredentialsReceived = false; // 플래그 초기화
    wifiStatus = true;
    connectToWiFi(); // Wi-Fi 연결
  }
  if (wifiStatus) {
    if (!client.connected()) {
      reconnectAWS(); // MQTT 재연결 시도
    }
  }
