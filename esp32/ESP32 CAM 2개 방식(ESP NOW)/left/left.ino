...
char action[32] = "capture"; // 별도로 전송할 action 데이터
...
void grabImage(){
  ...
  IPAddress broadcastIP = ~WiFi.subnetMask() | WiFi.gatewayIP(); // 브로드캐스트 주소 계산
  String message = "capture";
  udp.beginPacket(broadcastIP, udpPort);
  udp.print(message);
  udp.endPacket();

  Serial.println("Capture command broadcasted.");
...
// Setup LED FLash if LED pin is defined in camera_pins.h
  ...
  strncpy(myData.ssid, WiFi.SSID().c_str(), sizeof(myData.ssid));
  strncpy(myData.password, WiFi.psk().c_str(), sizeof(myData.password));
  ...
  // 피어 추가
  esp_now_del_peer(peerAddress); // 기존 피어 삭제
  esp_now_peer_info_t peerInfo;
  memset(&peerInfo, 0, sizeof(peerInfo));
  memcpy(peerInfo.peer_addr, peerAddress, 6);
  peerInfo.channel = WiFi.channel(); // 현재 채널
